...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '' || $prenom === '') {
        ...
        if ($cne !== '') {
            $stmt = $pdo->prepare("SELECT id FROM stagiaires WHERE cne = ?");
            $stmt->execute([$cne]);
            if ($stmt->fetch()) {
    ...
    if (!$erreur) {
        $stmt = $pdo->prepare("INSERT INTO stagiaires 
            (nom, prenom, cne, lieu_residence, telephone, email, etablissement, specialite, duree_stage, admin_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$nom, $prenom, $cne ?: null, $lieu, $telephone, $email, $etablissement, $specialite, $duree, $_SESSION['admin_id']]);

        header("Location: liste_stagiaires.php?msg=added");
        exit();
    }
}
?>
...
<head>
    ...
    <title>إضافة متدرب - Ajouter un stagiaire</title>
...
<div class="contenu">
    <div class="panel panel-default panel-formulaire">
        <div class="panel-body">
            ...
            <h2 style="color:#0ea5e9; margin-top:10px;">إضافة متدرب / Ajouter un stagiaire</h2>
            ...
            <form method="POST" action="ajouter_stagiaire.php">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="nom" class="form-control" maxlength="50" required>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>الاسم الشخصي * <span class="fr">/ Prénom</span></label>
                            <input type="text" name="prenom" class="form-control" maxlength="50" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="cne" class="form-control" maxlength="8" pattern="[A-Za-z]{2}[0-9]{6}" title="حرفين كبيرين متبوعين بـ 6 أرقام (مثال: QV123413)" style="text-transform:uppercase;" placeholder="مثال: QV123413">
                </div>

                <div class="form-group">
                    <label>مكان السكن <span class="fr">/ Lieu de résidence</span></label>
                    <input type="text" name="lieu_residence" class="form-control" maxlength="150">
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="email" name="email" class="form-control" maxlength="100" placeholder="exemple@gmail.com">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="etablissement" class="form-control" maxlength="150" placeholder="مثال: EST Béni Mellal">
                </div>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="specialite" class="form-control" maxlength="100">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>مدة التداريب <span class="fr">/ Durée du stage</span></label>
                            <input type="text" name="duree_stage" class="form-control" maxlength="50" placeholder="مثال: شهرين / 2 mois">
                        </div>
                    </div>
                </div>

                <button type="submit" class="btn btn-lg btn-block" style="background:#0ea5e9; color:white;">
                    <span class="glyphicon glyphicon-ok"></span> إضافة متدرب / Ajouter
                </button>
            </form>
        </div>
    </div>
